/* 
   ...
   compile with "CC -o product product.C -lcomplex -lm"
...
int main(int argc, char *argv[]) {
  ...
  if(argc != 5) {
    printf("Usage : product <n1> <n2> <nu> <nv>\n");
    return(1);
  }
  ...
  sprintf(filename, "product%1d%1d.list", n1, n2);
  ...
  dtheta = 2.0*M_PI/(double) (nu-1);
  dphi = PHI*2.0/(double) (nv-1);

  for(k1=0; k1<n1; k1++) {
    for(k2=0; k2<n2; k2++) {
      sprintf(filename, "prod%1d%1d-%1d%1d.mesh", n1, n2, n1, n2, k1, k2);
	...
	theta = 0.0;
	for(i=0; i<nu; i++) {
	  tmp = complex(phi/n1, theta/n1);
	  z1 = s(k1,n1)*exp(tmp);
	  tmp = complex(-phi/n2, -theta/n2);
	  z2 = s(-k2,n2)*exp(tmp);
	  theta += dtheta;
	  x = real(z1);
	  y = imag(z1);
